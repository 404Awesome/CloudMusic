    ...
    <template #default>
      <ul grid3Cols py-4>
        <li v-for="item in collectorList" :key="item.userId" flex overflow-hidden gap-10px>
          <!-- 头像 -->
          <el-avatar @click="$router.push(`/otherHomePage/${item.userId}`)" :src="item.avatarUrl" cursor="pointer"
            :size="70" />
          <!-- 信息 -->
          <div class="group" flex overflow-hidden flex-1 flex-col flex-nowrap justify-center>
            <div flex items-center gap-5px>
              <!-- 名字 -->
              <p @click="$router.push(`/otherHomePage/${item.userId}`)" truncate text-15px cursor-pointer
