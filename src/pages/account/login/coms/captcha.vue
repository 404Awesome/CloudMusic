<!-- 密码登陆 -->
<template>
  <div ref="captchaEl" w-270px mx-auto>
...
let submit = () => {
  formEl.value.validate(async (isValid: boolean) => {
    if (isValid) {
      ...
      // 发起登陆请求
      let { code, cookie }: any = await AccountAPI.loginCaptcha(parseInt(formData.phone), parseInt(formData.captcha));
      if (code == 200) {
        // 登陆成功
